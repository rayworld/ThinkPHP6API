...
class Common extends Controller {
  ...
  public function __construct() {
    ...
    //参数过滤
    $this->params = $this->validateParams($this->request->param(true));
  }
   ...
   * @return array [过滤后的参数]
   */
  public function validateParams($arr) {
    ///V1版
    //取得控制器名，去掉前面的版本号
    ...
    if (!$this->validater->check($arr)) {
      //输出错误信息
      $this->returnMsg(404, $this->validater->getError());
    }
    //返回过滤后的参数
    return $arr;
  }
  ...
  public function checkToken($arr) {
    ...
    if ($client_token !== $server_token) {
      //打印错误信息
      $this->returnMsg(403, 'token error');
    }
  }
   ...
   */
  public function checkAccountType($account_name) {
    //验证是不是email
   ...
   * @return void [空]
   */
  public function checkUserExist($account_name, $account_type, $is_exist) {
    ...
    case 2:
      if ($mobile_res) {
        $this->returnMsg(400, '手机号被占用');
      }
      break;
    case 3:
      if (!$mobile_res) {
        $this->returnMsg(400, '手机号不存在');
      }
      break;
    case 4:
      if ($email_res) {
        $this->returnMsg(400, '邮箱被占用');
      }
      break;
    case 5:

      if (!$email_res) {
        $this->returnMsg(400, '邮箱不存在');
      }
      break;
    }
  }
   ...
   * @return void [空]
   */
  public function checkCaptcha($account_name, $captcha) {
    //从Seesion取得该账号的验证码。
  ...
  public function uploadFile($file, $type = '') {
    ...
    if ($info) {
      ...
      $this->returnMsg(400, $info->getError());
    }
    if (!empty($type)) {
      $this->cutImage($path);
    }
  ...
  public function validateRequest($method) {
    ...
    case 'get':
      if (!request()->isGet()) {
        $this->returnMsg(400, 'Api数据请求方式错误！');
      }
      break;
    case 'post':
      if (!request()->isPost()) {
